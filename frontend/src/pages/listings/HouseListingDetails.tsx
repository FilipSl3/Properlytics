...
export default function HouseListingDetails() {
  ...
  const title = useMemo(() => pickTitle(item, "Dom"), [item]);
  ...
  const params: ParamRow[] = useMemo(() => {
    ...
    if (item.plot_area != null) rows.push({ label: "Pow. działki", value: `${item.plot_area} m²` });
    if (item.garage != null) rows.push({ label: "Garaż", value: Number(item.garage) === 1 ? "Tak" : "Nie" });
  ...
  return (
    <div className="space-y-4">
      <Link to="/ogloszenia/domy" className="text-sm text-gray-600 hover:text-gray-900">
        Wróć do listy domów
      </Link>
